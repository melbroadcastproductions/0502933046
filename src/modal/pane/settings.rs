...
pub fn kline_cfg_view<'a>(
    ...
    let content = match kind {
        KlineChartKind::Candles => {
            let trade_bubbles_section = {
                let bubbles_checkbox = tooltip(
                    checkbox(cfg.show_trade_bubbles)
                        .label("Show trade bubbles")
                        .on_toggle(move |value| {
                            Message::VisualConfigChanged(
                                pane,
                                VisualConfig::Kline(data::chart::kline::Config {
                                    show_trade_bubbles: value,
                                    ..cfg
                                }),
                                false,
                            )
                        }),
                    Some("Overlay size-scaled trade circles (buy/sell) on the candlestick chart"),
                    TooltipPosition::Top,
                );

                let size_filter_slider = labeled_slider(
                    "Trade",
                    0.0..=50000.0,
                    cfg.trade_bubble_size_filter,
                    move |value| {
                        Message::VisualConfigChanged(
                            pane,
                            VisualConfig::Kline(data::chart::kline::Config {
                                trade_bubble_size_filter: value,
                                ..cfg
                            }),
                            false,
                        )
            ...
            let big_orders_section = {
                let big_orders_checkbox = tooltip(
                    checkbox(cfg.show_big_order_levels)
                        .label("Show major order book levels")
                        .on_toggle(move |value| {
                            Message::VisualConfigChanged(
                                pane,
                                VisualConfig::Kline(data::chart::kline::Config {
                                    show_big_order_levels: value,
                                    ..cfg
                                }),
                                false,
                            )
                        }),
                    Some("Display key major resting order book levels (limit bids/asks) on candlestick chart"),
                    TooltipPosition::Top,
                );
                ...
                let big_order_filter_slider = labeled_slider(
                    ...
                    move |value| {
                        Message::VisualConfigChanged(
                            pane,
                            VisualConfig::Kline(data::chart::kline::Config {
                                big_order_level_threshold: value,
                                ..cfg
                            }),
                            false,
                        )
